...
export async function hasJoined(serverId, username) {
    ...
    const props = await buildProperties(serverSession.username, serverSession.selectedProfile, serverSession.username);
    return {
...
export async function getProfile(id) {
    ...
    const props = await buildProperties(serverSession.username, id, serverSession.username);
    return {
        id,
        name: serverSession.username,
        properties: props
...
async function signValue(value, privatePem) {
    if (!privatePem) {
        return '';
    }
    const key = await crypto.subtle.importKey('pkcs8', pemToArrayBuffer(privatePem), { name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-1' }, false, ['sign']);
