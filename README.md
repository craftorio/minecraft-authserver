...
## Supabase setup
Run `supabase init` to create a local project, then apply the migration from `supabase/migrations/0001_initial.sql` to create the tables and storage bucket. The example configuration in `supabase/config.toml` sets local ports and a placeholder JWT secret.

## Docker
A `Dockerfile` and `docker-compose.yml` are provided for convenience. The compose stack starts a Supabase instance and runs the worker using Wrangler.
